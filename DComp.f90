...
call input()
!
    do i = PDoperator%n0, PDoperator%n1, PDoperator%nskip
       mask(i) = 1
    enddo
!
    ngrad = 0
...
!
      ...
      enddo
      ij = 0
      do i = 1 , PDoperator.totnode
         read(2,*) coord(i,1), coord(i,2), bluevec_org(i)
            !red = 128.0d0*dsin(3.0d0*pi*xp)*dcos(2.0d0*pi*yp)
            !igreen = 0
            !iblue = 0
            !ialfa = 0
            redvec_org(i) = 0.0d0
            greenvec_org(i) = 0.0d0
            alfavec_org(i) = 0.0d0
            coord(i,3) = 0.0d0
            dvolume(i) = 1.0d0
         enddo
      enddo
      close(2)
      read(1,*) PDoperator%n0, PDoperator%nskip
      PDoperator%n1 = PDoperator.totnode
!
...
!
    ...
    do i = 1 , totnode
       ...
       !endif
       write(12,"(3(i7,3x),2x,4(I3,3x),2x,8(e16.9,3x),3x,i6)") (idnint(coord(i,j)),j=1,3), redvec(i,1), greenvec(i,1),bluevec(i,1), alfavec(i,1), &
