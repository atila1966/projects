...
real *8, allocatable :: dvolume(:), deltax(:), deltay(:), deltaz(:)
...
!
      ...
      implicit none
      integer i
      real *8, allocatable :: c20vec(:), c02vec(:), c00vec(:), c11vec(:), c10vec(:), c01vec(:), fvec(:)
...
!
      ...
      do i = 1 , totnode
         fvec(i) = 0.0d0
         if(dsqrt(coord(i,1)**2+coord(i,2)**2) <= 1.0d0) then
            fvec(i) =(coord(i,1)**2 + coord(i,2)**2  - 2.0d0)**3
         endif
